...
// RUN: %hermes -type-annotation-file=%annotation_file -gc-init-heap=4M -O -Xhermes-internal-test-methods %s | %FileCheck --match-full-lines %s
// RUN: %hermes -type-annotation-file=%annotation_file -O -emit-binary -out %t.hbc %s && %hermes -type-annotation-file=%annotation_file -gc-init-heap=4M -Xhermes-internal-test-methods %t.hbc | %FileCheck --match-full-lines %s
// RUN: %shermes -type-annotation-file=%annotation_file -exec %s -Wx,-gc-init-heap=4M,-Xhermes-internal-test-methods | %FileCheck --match-full-lines %s
