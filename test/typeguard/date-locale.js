...
/*
RUN: TZ=EST+5 LC_ALL=en_US _HERMES_TEST_LOCALE=en_US %hermes -type-annotation-file=%annotation_file -O -target=HBC %s \
RUN:            | %FileCheck --match-full-lines -check-prefix US %s
RUN: TZ=EST+5 LC_ALL=tr_TR _HERMES_TEST_LOCALE=tr_TR %hermes -type-annotation-file=%annotation_file -O -target=HBC %s \
RUN:            | %FileCheck --match-full-lines -check-prefix TR %s
RUN: TZ=EST+5 LC_ALL=tr_TR _HERMES_TEST_LOCALE=tr_TR %shermes -type-annotation-file=%annotation_file -exec %s \
RUN:            | %FileCheck --match-full-lines -check-prefix TR %s
