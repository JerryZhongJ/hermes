...
/// TypeGuard instruction: checks if a value matches an expected type at runtime.
/// This is a terminator instruction that branches to one of two successors:
/// - trueDest: taken if the value matches the expected type (speculative path)
/// - falseDest: taken if the value doesn't match (fallback path)
class TypeGuardInst : public TerminatorInst {
  TypeGuardInst(const TypeGuardInst &) = delete;
  void operator=(const TypeGuardInst &) = delete;

  /// The expected type for the type guard check.
  Type expectedType_;

 public:
  enum { CheckedValueIdx, TrueBlockIdx, FalseBlockIdx };
  ...
  explicit TypeGuardInst(
      Value *checkedValue,
      Type expectedType,
      BasicBlock *trueDest,
      BasicBlock *falseDest)
      : TerminatorInst(ValueKind::TypeGuardInstKind),
        expectedType_(expectedType) {
    pushOperand(checkedValue);
    pushOperand(trueDest);
    pushOperand(falseDest);
  }

  explicit TypeGuardInst(
      const TypeGuardInst *src,
      llvh::ArrayRef<Value *> operands)
      : TerminatorInst(src, operands),
  ...
  static bool classof(const Value *V) {
    return V->getKind() == ValueKind::TypeGuardInstKind;
  }
  ...
  BasicBlock *getSuccessorImpl(unsigned idx) const {
    ...
    if (idx == 1)
      return getFalseDest();
    llvm_unreachable("TypeGuardInst only has 2 successors!");
  }

  void setSuccessorImpl(unsigned idx, BasicBlock *B) {
    assert(idx <= 1 && "TypeGuardInst only has 2 successors!");
    setOperand(B, idx + TrueBlockIdx);
  }
};
